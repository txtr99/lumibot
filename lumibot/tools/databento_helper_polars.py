...
def _format_futures_symbol_for_databento(asset: Asset, reference_date: datetime = None) -> str:
    ...
    Notes
    -----
    TopStepX Contract Override:
        Some brokers (like TopStepX) only offer certain contract months that may differ
        from CME's standard "front month" contract. For example, TopStepX may offer
        February 2026 (G26) for MGC while CME's front month is December 2025 (Z25).

        When TOPSTEPX_CONTRACT_OVERRIDE is set for a symbol, we use Databento's
        continuous contract second position (.c.1) instead of front month (.c.0)
        to match the broker's actual tradeable contract.

        This mapping should be updated when contracts roll or broker offerings change.
    """
    ...
    # month is still Z25 (Dec 2025), causing a ~$40 price mismatch if using .c.0
    # TODO: Make this configurable via env var or config file for easier updates
    TOPSTEPX_CONTRACT_OVERRIDE = {
        "MGC": "MGCG6",  # Use G26 (Feb 2026) instead of Z25 (Dec 2025)
        "GC": "GCG6",  # Same for full-size gold futures
    }
    ...
    # Check for TopStepX contract override (live trading only - no reference_date)
    if reference_date is None and symbol in TOPSTEPX_CONTRACT_OVERRIDE:
        override_symbol = TOPSTEPX_CONTRACT_OVERRIDE[symbol]
        logger.info(f"Using TopStepX contract override: {symbol} -> {override_symbol}")
