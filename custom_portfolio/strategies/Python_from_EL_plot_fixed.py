...
# Global toggles for CSV and plot components
...
def export_data_to_csv(collected_data, symbol_root: str, csv_filename: str = "latestplot.csv"):
    ...
    try:
        ...
        if hasattr(self, "vars") and hasattr(self.vars, "data_fetch_count"):
            total_requests = self.vars.data_fetch_count + self.vars.cache_hit_count
            cache_hit_rate = (self.vars.cache_hit_count / total_requests * 100) if total_requests > 0 else 0
            print("\n📊 Cache Performance Summary:")
            print(f"  Data fetches: {self.vars.data_fetch_count}")
            print(f"  Cache hits: {self.vars.cache_hit_count}")
...
if __name__ == "__main__":
    ...
    if IS_BACKTESTING:
        ...
        if ENABLE_CSV_EXPORT or ENABLE_PLOTTING:
            ...
            if ENABLE_CSV_EXPORT:
                ...
                # Still need df for plotting if CSV is disabled but plotting is enabled
                if ENABLE_PLOTTING and FuturesOneSidedATRStrategy._collected_data is not None:
                    if len(FuturesOneSidedATRStrategy._collected_data) > 0:
                        df = pd.concat(FuturesOneSidedATRStrategy._collected_data)
                        df = df[~df.index.duplicated(keep="last")]
                        df = df.sort_index()
                        success = True

            if ENABLE_PLOTTING and success and df is not None:
